    ...
    <div class="w-100">
        <v-card class="w-100 pa-5 my-3" v-for="option, option_index in options" :key="option_index">
            <v-row class="align-center">
                <v-col cols="3">
                    <p class="fw-400">ตัวเลือก {{ option_index + 1 }}</p>
                </v-col>
                <v-col cols="8">
            ...
            <v-row v-for="item, item_index in option.items" :key="item_index" class="align-center">
                ...
                <v-col cols="4" class="pr-0">
                    <v-text-field placeholder="เช่น ไก่..." v-on:input="(event) => handleInput(event, option_index)" v-model="item.item_name" hide-details density="compact" variant="outlined"></v-text-field>
                </v-col>
                <v-col cols="2" class="text-center">
                    <p class="fw-400">ราคา</p>
                </v-col>
                <v-col cols="2" class="pl-0">
                    <v-text-field v-model="item.item_price" hide-details density="compact" variant="outlined"></v-text-field>
                </v-col>
                <v-col cols="1" class="pa-0" v-if="item_index != option.items.length-1">
                    <v-btn @click="removeItem(option_index, item_index)" icon size="x-small" elevation="0"><v-icon size="large">mdi-trash-can-outline</v-icon></v-btn>
                </v-col>
            </v-row>
        </v-card>
        <v-btn color="foodhub" class="w-100" prepend-icon="mdi-plus" @click="addOption()">เพิ่มตัวเลือกสินค้า</v-btn>
    </div>
</template>
