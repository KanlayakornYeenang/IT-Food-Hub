            ...
            <div style="margin-bottom: 6vh; border-bottom: 1px solid grey;">
                <v-col>
                    <div style="font-size: 20px;">
                        เปลี่ยนรหัสผ่าน {{ user.user_password }}
                    </div>
                    ...
                    <v-row>
                        ...
                        <v-col>
                            ...
                            <template v-if="v$.user_Password.$error">
                                ...
                                <p style="color: red;" v-if="v$.confirm_Password.required.$invalid">
...
export default {
    ...
    validations() {
        return {
            ...
            confirm_Password: { sameAs: sameAs(this.new_Password), minLength: minLength(8), required: required }, // Matches อันใหม่
            user_Password: { sameAs: sameAs(this.user.user_password), required: required }// Matches อันเก่าใน db
        }
    },
    setup() {
    ...
    methods: {
        submit() {
            ...
            if (!this.v$.$invalid) {
                ...
                axios.put("api/updatepassword", data).then(res => {
                    console.log(res)
                    this.$router.go() 
                }).catch(err=>{
                    console.log(err)
